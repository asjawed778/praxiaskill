...
const AssignCourse = ({ open, onClose, user }) => {
    ...
    const [assignCourse, {isLoading: assignCourseLoading}] = useAssignCourseMutation();
    const { control, handleSubmit, reset } = useForm({
        resolver: yupResolver(assignCourseSchema),
    });
    
    // Safely extract course data from API response
    const courseData = data?.data?.courses || data?.courses || [];
    const courseOptions = courseData.map((course) => ({
        label: course?.title,
        value: course?._id,
    }));
    console.log("Course data: ",courseOptions);
    ...
    const onSubmit = async (formData) => {
        try {
          ...
          toast.error(error?.data?.message || "Failed to assign course. Please try again.");
        }
      };
