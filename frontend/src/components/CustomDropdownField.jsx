...
const CustomDropdownField = ({
  ...
  const getValues = formContext?.getValues ?? (() => undefined);
  const value = isUsingHookForm ? getValues(name) : propValue;
  const { colors } = useAppTheme();

  const [fetchEnabled, setFetchEnabled] = useState(false);

  useEffect(() => {
    if (endpoint && value && !fetchEnabled) {
      setFetchEnabled(true);
    }
  }, [endpoint, value, fetchEnabled]);
